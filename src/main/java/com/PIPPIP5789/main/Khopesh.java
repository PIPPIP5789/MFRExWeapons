 ...
 */
public class Khopesh extends ItemWeaponMFR {
     ...
     */
    public Khopesh(String name, Item.ToolMaterial material, int rarity, float weight) {
    ...
    public void onProperHit(EntityLivingBase user, ItemStack weapon, Entity hit, float dam) {
        if (!user.world.isRemote && user.getRNG().nextInt(5) == 0) {
            TacticalManager.knockbackEntity(hit, user, 1.5F, 1.0F);
        }
    ...
    public float getAttackSpeed(ItemStack item) {
        return (super.getAttackSpeed(item) + speedMace) / 2;
    }
